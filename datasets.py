...
def load_data(config):
    ...
    if data_name in ['MNIST-USPS']:
        ...
        # np.random.seed(4485)
        #
        # perturbation_level = 0.02  # 扰动级别
        #
        # # 为视图 1 和 视图 2 添加轻微扰动
        # xx1 = x1 * (1 + np.random.uniform(-perturbation_level, perturbation_level, x1.shape))
        # xx2 = x2 * (1 + np.random.uniform(-perturbation_level, perturbation_level, x2.shape))
