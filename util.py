...
def next_batch(X1, X2, X3, X4, batch_size):
    ...
    for i in range(int(total)):
        ...
        if end_idx - start_idx < batch_size:
            ...
            batch_x1 = F.pad(batch_x1, (0, 0, 0, pad_size))
            batch_x2 = F.pad(batch_x2, (0, 0, 0, pad_size))
            batch_x3 = F.pad(batch_x3, (0, pad_size))
            batch_x4 = F.pad(batch_x4, (0, pad_size))
...
def cal_std(logger, *arg):
    ...
    print(output)
    return np.mean(arg[0]),np.mean(arg[1]),np.mean(arg[2])
